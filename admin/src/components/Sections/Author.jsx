...
import { getAuthors } from "../../services/getProducts";
...
function Author({ updateEntry }) {
  const [data, setData] = useState([]);
  const [totalPage, setTotalPage] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);

  useEffect(() => {
    getAuthors(setData, setTotalPage, currentPage);
  }, []);
      ...
      <table className="w-full table-auto">
        <thead>
          <tr>
            ...
            <th className="py-2 text-white">Department</th>
...
function AuthorSelection({ item, updateEntry, index, currentPage }) {
  ...
  function deleteProduct(id) {
    axios
      .delete(import.meta.env.VITE_BACKEND_URL + `/author/${id}`)
      .then((res) => {
        console.log("Deleted successfully", res.data);
        setShowDelete(null);
      })
      .catch((error) => {
        console.error("Error in deleting the auhtor", error.message);
      });
    ...
    <tr key={item.id} className="text-center">
      ...
      <td className="border-t border-gray-200 py-2 flex justify-center space-x-2">
        <button
          ...
          onClick={() => handleUpdate(item.author_id)}
        >
          Update
        </button>
        <button
